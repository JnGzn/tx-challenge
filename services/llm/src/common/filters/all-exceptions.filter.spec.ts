...
describe('AllExceptionsFilter', () => {
  ...
  beforeEach(() => {
    ...
    host = {
      switchToHttp: () => ({
        getResponse: () => ({ status }),
        getRequest: () => ({ method: 'GET', url: '/explanations' })
      })
    };
  });
  ...
  describe('Prisma known errors', () => {
    ...
    it('P2002 array target hits FIELD_LABELS', () => {
      filter.catch(build('P2002', { target: ['transactionId'] }), host);
      expect(json).toHaveBeenCalledWith({
        error: 'Conflict',
        message: 'El identificador de transacción ya se encuentra registrado.'
      });
    });

    it('P2002 string target with underscore', () => {
      filter.catch(build('P2002', { target: 'Event_transactionId_key' }), host);
      expect(json).toHaveBeenCalledWith({
        error: 'Conflict',
        message: 'El identificador de transacción ya se encuentra registrado.'
      });
